...
added_py="$(bash scripts/change-scope.sh --status --ext py | awk -F'\t' '$1=="A"{print $2}' | grep '^backend/open_webui/' | grep -v '/test_' | grep -v '/tests/' | grep -v '^backend/open_webui/test/' | grep -v '/test_ocu_harness\.py$' || true)"
