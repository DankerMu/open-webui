...
| WebUI    | `/Users/danker/Documents/31308/open-webui-ocu`    | tag `v0.11.3`，`2a960a59fe1dbbd35282f0556b3666d81102e781`；fork `DankerMu/open-webui` 分支 `main` |
...
- 静态相对资源的受限资源地址/重写器推迟到后续 change（2026-09-20 修订：本计划内生成页面只保证内联资源；方案与未决项见 openspec design D19 与 `docs/decisions/proposed/`），届时只读范围仍不含终端或浏览器控制。禁止通配 CORS 带凭据访问（当前 OCU 为 `allow_origins=["*"]`，`app.py:235`；反代收口后 OCU 同步改为仅允许 WebUI origin）。
...
| ID    | 场景与操作                                                                                                                                                                                       | 通过标准                                                                                                                                                                                                                                                                                   |
| ----- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| A-T01 | HTML 含按钮、JS、内联 CSS 与 data-URI 图片（相对路径子资源为已记录限制，见 openspec design D19），修改后刷新；`iframeSandboxAllowSameOrigin=true`；**同一文件分别经侧栏 iframe、点击消息内链接顶层打开、新标签直接输入 `/ocu/files/...` URL、SVG 内嵌 `<script>`** | 可交互、内联资源齐全、旧缓存不残留；三种打开方式下生成页面均读不到 WebUI token/localStorage、调不到执行 API、`document.origin` 为 opaque；响应带 `CSP: sandbox allow-scripts allow-forms` 与 `nosniff`，页面内 JS 仍可执行                                                                     |
...
| A-T09 | 登出/撤权时已有 WS；内部令牌缺失或错误                                                                                                                                                           | 撤权（删除用户/失去所有权）及时关闭；登出不吊销已建立连接（无 Redis，2026-09-20 修订，见 openspec design D18）；无令牌请求拒绝；凭据不出现在聊天、Referer 和普通日志                                                                                                                                                                                                                         |
