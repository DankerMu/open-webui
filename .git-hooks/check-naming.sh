...
# Wired into pre-commit via .pre-commit-config.yaml; Claude Code write-time via
# .claude/hooks/pre-write-naming.sh. See AGENTS.md § Code Canonicality.
set -euo pipefail

# Regex source: repo-root constraints.yaml (preferred) or fallback defaults.
# Must run on bare macOS bash 3.2 — no yq, no mapfile.
...
if [ -f "$constraints_file" ]; then
  ...
  while IFS= read -r pat; do
    [ -z "$pat" ] && continue
    pat="${pat%\$}"
    if [ -z "$forbidden_joined" ]; then forbidden_joined="$pat"; else forbidden_joined="$forbidden_joined|$pat"; fi
  done <<EOF2
...
EOF2
  ...
  if [ -n "$scratch_paths_line" ]; then
    ...
    for term in $scratch_cleaned; do
      term="${term#/}"; term="${term%/}"
      [ -z "$term" ] && continue
      if [ -z "$scratch_joined" ]; then scratch_joined="$term"; else scratch_joined="$scratch_joined|$term"; fi
    done
...
What to do:
  - Refactoring: rename the new file over the old one; git tracks history.
  - Genuinely separate concept: rename to something descriptive (no suffix).
  - Throwaway exploration: keep it outside the repo (e.g., /tmp).

See AGENTS.md § Code Canonicality for the full rule.
EOF2
  exit 1
fi
echo "check-naming.sh: staged paths conform." >&2
exit 0
