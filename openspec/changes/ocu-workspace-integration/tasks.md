...
- [ ] 7.1 `OCU_PUBLIC_PREFIX` env (default `""`): `_generate_preview_html` emits every URL (stylesheets, scripts, `apiUrl`, `filesBase`) under the prefix, plus `describeUrl` = `/api/v1/ocu/workspaces/{chat_id}` (WebUI's same-origin describe route, emitted by the shell so the SPA never constructs a WebUI address); static mount at `{prefix}/static/`; `browser-viewer.js` address adaptation. Issue17 moves the original inline heartbeat into the wrapper-managed SPA effect with the same prefix.
...
- [x] 14.2 `deploy/firewall/docker-user-rules.sh` and `deploy/firewall/check.sh`: explicit destination allowlist and default deny for sandbox-originated forwarded/host-local traffic, control-plane/metadata hard denies, protected CDP/ttyd replies, IPv6 bypass prevention, idempotent owned rules and ordered-policy verification. Both run after network provisioning and before service starts. Development evidence is archived in `2026-09-25-ocu-sandbox-egress-guard`; real kernel/Docker acceptance is deferred to issue36 by user decision. Full egress closure also requires group21 / issue79.
- [x] 14.3 Retention guard: stop only, keep volume/dirs, explicit launch after stop; internal token, `/ocu` prefix, no-autostart, public base and internal WebUI auth URL provisioned privately without the historical CLI patch. Development evidence is archived in `2026-09-25-ocu-retention-env`; actual service startup and168-hour retained-data acceptance remain issue36 by user decision, with image provenance issue33 and mandatory DNS issue79 unchanged.
