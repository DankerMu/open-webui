...
Run the configured OCU non-Docker suite and structure check; CI configuration collects root auth tests and exercises the packaged startup/MCP path. Per the user's 2026-09-22 instruction, Docker-backed tests run once after all epic development, before final acceptance, rather than blocking each issue's merge. Unexecuted Docker checks remain pending in the epic acceptance ledger; mocks do not replace image evidence. The fork's manually disabled build workflow is not evidence of a passing runtime check.
